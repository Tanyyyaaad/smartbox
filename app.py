...
Полностью рабочий вариант без ошибок кодировки в QR.
"""
...
def generate_qr_code(box_id, box_number):
    ...
    В центре отображается только номер коробки (чтобы избежать проблем с кириллицей).
    """
    ...
    # Используем стандартный шрифт (он поддерживает только латиницу, но нам хватит для цифр)
    try:
    ...
    # Определяем размер текста для центрирования
    bbox = draw.textbbox((0, 0), text, font=font)
    w = bbox[2] - bbox[0]
    h = bbox[3] - bbox[1]

    # Позиция по центру изображения
    x = (img.width - w) // 2
    y = (img.height - h) // 2
