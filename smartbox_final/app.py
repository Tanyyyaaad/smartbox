...
def get_next_box_number(user_id):
    last = Box.query.filter_by(user_id=user_id).order_by(Box.box_number.desc()).first()
...
def generate_qr_code(box_id, box_number):
    ...
    sign_w, sign_h = sign_x2 - sign_x1, sign_y2 - sign_y1
    ...
    qr_img.paste(cat, ((width - cat.size[0])//2, (height - cat.size[1])//2), cat)
    buf = io.BytesIO()
    qr_img.save(buf, 'PNG')
    buf.seek(0)
    return buf
...
def save_photo(file):
    if file and allowed_file(file.filename):
        ...
        uname = f"{datetime.utcnow().strftime('%Y%m%d%H%M%S%f')}_{fname}"
    ...
    if request.method == 'POST':
        ...
        for file in uploaded_files:
            if file and file.filename:
                ...
                if saved_name:
                    db.session.add(BoxPhoto(box_id=box.id, filename=saved_name))
        db.session.commit()
        qr_img = generate_qr_code(box.id, box.box_number)
        return send_file(qr_img, mimetype='image/png', as_attachment=True, download_name=f'qr_box_{box.box_number}.png')
...
def not_found(e): return render_template('base.html', error='Страница не найдена'), 404
@app.errorhandler(403)
def forbidden(e): return render_template('base.html', error='Доступ запрещён'), 403
